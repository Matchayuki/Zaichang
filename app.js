...
function renderJourney(){
  ...
  $('step-count').textContent=step+' / 3';$('make-reply').disabled=!r.capacity||!r.source.trim()||aiBusy;$('capacity-prompt').hidden=Boolean(r.capacity)&&Boolean(r.source.trim());$('capacity-prompt').textContent=!r.source.trim()?'请先返回第一步，放入朋友的原话。':'先选一个你现在的状态，就可以继续了。';
